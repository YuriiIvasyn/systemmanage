...
$object = $db->select("*")->from("users")->where('id = ?', $user_id)->execute()->fetchCollection()[0];
if($object->u_access_level != 1){
...
if (isset($_POST['u_name']) && isset($_POST['u_last_name']) && isset($_POST['u_email']) && isset($_POST['u_pass_hash'])) {
    $the_sault = '#4ds$';

    if(isset($_POST['u_pass_hash']) && $_POST['u_pass_hash']!=""){
        $_POST['u_pass_hash'] = md5($_POST['u_pass_hash'].$the_sault);
    }
    $_POST['u_access_level']=2;
...
<div class="wrapper">
    <main>
        ...
        <form class="ui big form" method="post">
            <div class="field">
                <label>Name</label>
                <input type="text" name="u_name" placeholder="Insert agent's name" >
            </div>
            <div class="field">
                <label>Surame</label>
                <input type="text" name="u_last_name" placeholder="Insert agent's surname" >
            </div>
            <div class="field">
                <label>email</label>
                <input type="email" name="u_email" placeholder="Insert agent's email" >
            </div>
            <div class="field">
